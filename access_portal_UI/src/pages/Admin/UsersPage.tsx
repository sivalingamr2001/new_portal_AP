...
export const UsersPage = () => {
  const usersConfig = useMemo<DynamicPageConfig<PortalUserDetails>>(() => ({
    ...
    fetchData: async (page, pageSize, search) => {
      return await usersApi.getPortalUsers({ page, pageSize, search: search || undefined })
    },
    ...
    columns: ({ openEditModal, toggleRowExpansion, expandedRowIds }) => [
          ...
          return (
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-indigo-600" onClick={() => toggleRowExpansion(uId)}>
                  {isExpanded ? <ChevronUp className="h-4 w-4" /> : <TextQuote className="h-4 w-4" />}
                </Button>
              </TooltipTrigger>
              <TooltipContent side="top">{isExpanded ? "Collapse Details" : "Expand Details"}</TooltipContent>
            </Tooltip>
          )
        },
      },
      ...
      { headerName: "Department Name", field: "department.name" },
      {
        headerName: "Actions",
        width: 100,
        cellRenderer: (params: any) => params.data && !params.data.__isDetailRow && (
          <Button size="sm" variant="outline" className="h-7 px-2 text-xs" onClick={() => openEditModal(params.data)}>Edit</Button>
        ),
      },
    ],
  }), [])

  return <DynamicGridPage config={usersConfig} ModalComponent={EditUsersModal} />
}
