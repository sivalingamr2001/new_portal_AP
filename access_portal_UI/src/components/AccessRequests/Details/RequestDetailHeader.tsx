        ...
        <div className="grid flex-1 gap-3 md:grid-cols-4">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={step.id}
                className="relative flex items-center gap-3 rounded-2xl border border-border/70 bg-card p-3 shadow-sm"
              >
                <div
                  className={cn(
                    "flex h-9 w-9 shrink-0 items-center justify-center rounded-full border text-xs font-bold transition-all duration-300",
                    {
                      ...
                      "border-emerald-500 bg-emerald-500/10 text-emerald-600 shadow-[0_0_14px_rgba(16,185,129,0.18)]":
                        step.status === "inprogress",
                      "border-rose-500 bg-rose-500/10 text-rose-600":
                        step.status === "destructive",
                      "border-border bg-slate-100 text-slate-500 dark:bg-slate-800/80 dark:text-slate-400":
                        step.status === "idle",
                    }
                  )}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p
                    className={cn("truncate text-[11px] font-medium", {
                ...
                {!isLast && (
                  <ArrowRight className="absolute top-1/2 -right-2 hidden h-4 w-4 -translate-y-1/2 text-border md:block" />
                )}
              </div>
            )
          })}
